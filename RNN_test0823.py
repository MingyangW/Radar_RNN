...
#        print(labels[:100])
        accur_num = sess.run(accursum, feed_dict={x:data, y:label})
    return accur_num



if __name__ == "__main__":
    filepath = "E:\\Github\\CNN_tensorflow_180726\\data\\leida3"
    files=[]
...
#    print(train_data.shape, test_data.shape, train_label.shape, test_label.shape)
    ...
    for i in range(labels.shape[0] // batch):
        print(i)
#        print(RNN_model(datas[batch*i:batch*(i+1)], labels[batch*i:batch*(i+1)]))
        point = point + RNN_model(datas[batch*i:batch*(i+1)], labels[batch*i:batch*(i+1)])
    print(point/((labels.shape[0] // batch)* batch))